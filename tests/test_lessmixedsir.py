...
def set_params():
    ...
    TOTAL_POP = np.uint32(2**28)
    NUM_POPS = np.uint32(2)
    INF_EXPORT_FRAC = np.float32(0.0)
    ...
    parser.add_argument("-n", "--num_pops", type=np.uint32, default=NUM_POPS)
    parser.add_argument("-p", "--total_pop", type=np.uint32, default=TOTAL_POP)
    parser.add_argument("-f", "--inf_export_frac", type=np.float32, default=INF_EXPORT_FRAC)
    parser.add_argument("--filename", type=Path, default=Path(__file__).parent / "sir.csv")
...
def define_network(params):
    ...
    if params.num_pops > 1:
        connection = np.ones((params.num_pops,params.num_pops)) * (params.inf_export_frac / (params.num_pops - 1))
        np.fill_diagonal(connection, (1.0-params.inf_export_frac))
    else:
        connection = 1.0
    return connection
...
def test_spatial_seir(params):
    ...
    print(f"Creating a spatial SEIR model with {params.total_pop:_} individuals.")
    community = abc(
        params.total_pop, beta=params.beta, exp_mean=params.exp_mean, exp_std=params.exp_std, inf_mean=params.inf_mean, inf_std=params.inf_std
    )
    ...
    #initialize with random node ids.
    community.nodeid = _prng.integers(0, params.num_pops, size=params.total_pop, dtype=np.uint16)
    pops = np.zeros((params.num_pops,), int)
    np.add.at(pops, community.nodeid, 1)
    print(pops)
    ...
    initial = _prng.choice(np.where(community.nodeid == 0)[0], size=params.initial_infs, replace=False)
    ...
    forces = np.zeros(params.num_pops, dtype=np.float32)
    report = np.zeros((params.timesteps, params.num_pops), dtype=np.uint32)

    def transmission(community, timestep):
        contagion = np.zeros(params.num_pops, dtype=np.uint32)
    ...
    df.write_csv(Path("output") / params.filename)
    print("Wrote SEIR channels to 'spatial_seir.csv'.")
    sdf = pl.DataFrame(data=report, schema=[f"node{i}" for i in range(params.num_pops)])
    sdf.write_csv(Path("output") / Path(str(params.filename).replace(".csv", "_report.csv")))
